...
if map_maker.config.mapname == "" then
	map_maker.config.mapname = defaults.mapname
end
if map_maker.config.mapauthor == "" then
	map_maker.config.mapauthor = defaults.mapauthor
end
if map_maker.config.maptitle == "" then
	map_maker.config.maptitle = defaults.maptitle
end
if map_maker.config.barrier_r == 0 then
	map_maker.config.barrier_r = defaults.barrier_r
end
if map_maker.config.center == "" then
	map_maker.config.center = defaults.center
else
	map_maker.config.center = minetest.parse_json(storage:get_string("center"))
end
if map_maker.config.flags == "" then
	map_maker.config.flags = defaults.flags
else
	map_maker.config.flags = minetest.parse_json(storage:get_string("flags"))
end
...
local function check_step()
	for _, pos in pairs(map_maker.config.flags) do
...
minetest.register_node(":ctf_map:flag", {
	...
	after_place_node = function(pos)
		table.insert(map_maker.config.flags, vector.new(pos))
		storage:set_string("flags", minetest.write_json(map_maker.config.flags))
	end,
	on_destruct = function(pos)
		for i, v in pairs(map_maker.config.flags) do
			if vector.equals(pos, v) then
				map_maker.config.flags[i] = nil
				return
			end
		end
	end,
...
local function to_2pos()
	return {
			x = map_maker.config.center.x - map_maker.config.center.r,
			y = map_maker.config.center.y - map_maker.config.center.h / 2,
			z = map_maker.config.center.z - map_maker.config.center.r,
		}, {
			x = map_maker.config.center.x + map_maker.config.center.r,
			y = map_maker.config.center.y + map_maker.config.center.h / 2,
			z = map_maker.config.center.z + map_maker.config.center.r,
...
local function we_import(name)
	...
	if pos1 and pos2 then
		...
		map_maker.config.center = vector.divide(vector.add(pos1, pos2), 2)
		map_maker.config.center.r = r
		map_maker.config.center.h = size.y
		storage:set_string("center", minetest.write_json(map_maker.config.center))
	end
end
...
local function get_flags()
	...
	local positive = nil
	for _, pos in pairs(map_maker.config.flags) do
		pos = vector.subtract(pos, map_maker.config.center)

		if map_maker.config.barrier_rot == 0 and pos.x < 0 or pos.z < 0 then
			negative = pos
		end
...
local function get_flag_status()
	if #map_maker.config.flags > 2 then
		return "Too many flags! (" .. #map_maker.config.flags .. "/2)"
	elseif #map_maker.config.flags < 2 then
		return "Place more flags (" .. #map_maker.config.flags .. "/2)"
	else
		local negative, positive = get_flags()
		if positive and negative then
			return "Flags placed (" .. #map_maker.config.flags .. "/2)"
		else
...
local function show_gui(name)
	if not map_maker.config.mapauthor then
		map_maker.config.mapauthor = name
		storage:set_string("mapauthor", map_maker.config.mapauthor)
	end
	...
	local formspec = {
		...
		"field[0.4,1;1,1;posx;X;", map_maker.config.center.x, "]",
		"field[1.4,1;1,1;posy;Y;", map_maker.config.center.y, "]",
		"field[2.4,1;1,1;posz;Z;", map_maker.config.center.z, "]",
		"field[0.4,2;1.5,1;posr;R;", map_maker.config.center.r, "]",
		"field[1.9,2;1.5,1;posh;H;", map_maker.config.center.h, "]",
		...
		"field[0.4,4.3;1,1;barrier_r;R;", map_maker.config.barrier_r, "]",
		"dropdown[1.15,4.05;1,1;barrier_rot;X=0,Z=0;",
		map_maker.config.barrier_rot == "x" and 1 or 2, "]",
		...
		minetest.formspec_escape(map_maker.config.maptitle), "]",
		"field[0.4,7.3;8.5,1;initial;Stuff to give on (re)spawn, comma-separated itemstrings;",
		minetest.formspec_escape(map_maker.config.mapinitial), "]",
		"field[0.4,8.4;4.25,1;name;File Name;",
		minetest.formspec_escape(map_maker.config.mapname), "]",
		"field[4.625,8.4;4.25,1;author;Author;",
		minetest.formspec_escape(map_maker.config.mapauthor), "]",
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if fields.posx or fields.posy or fields.posz or fields.posr or fields.posh then
		map_maker.config.center.x = tonumber(fields.posx)
		map_maker.config.center.y = tonumber(fields.posy)
		map_maker.config.center.z = tonumber(fields.posz)
		map_maker.config.center.r = tonumber(fields.posr)
		map_maker.config.center.h = tonumber(fields.posh)
		storage:set_string("center", minetest.write_json(map_maker.config.center))
	end

	fields.barrier_r = tonumber(fields.barrier_r)
	if fields.barrier_r and fields.barrier_r ~= map_maker.config.barrier_r then
		map_maker.config.barrier_r = fields.barrier_r
		storage:set_int("barrier_r", map_maker.config.barrier_r)
	end

	if fields.title and fields.title ~= map_maker.config.maptitle then
		map_maker.config.maptitle  = fields.title
		storage:set_string("maptitle", map_maker.config.maptitle)
	end

	if fields.author and fields.author ~= map_maker.config.mapauthor then
		map_maker.config.mapauthor = fields.author
		storage:set_string("mapauthor", map_maker.config.mapauthor)
	end

	if fields.name and fields.name ~= map_maker.config.mapname then
		map_maker.config.mapname = fields.name
		storage:set_string("mapname", map_maker.config.mapname)
	end

	if fields.initial and fields.initial ~= map_maker.config.mapinitial then
		map_maker.config.mapinitial = fields.initial
		storage:set_string("mapinitial", map_maker.config.mapinitial)
	end

	if fields.barrier_rot and fields.barrier_rot ~= "" then
		map_maker.config.barrier_rot = fields.barrier_rot == "X=0" and "x" or "z"
		storage:set_string("barrier_rot", map_maker.config.barrier_rot)
	end

	if fields.set_center then
		local r = map_maker.config.center.r
		local h = map_maker.config.center.h
		map_maker.config.center = vector.round(player:get_pos())
		map_maker.config.center.r = r
		map_maker.config.center.h = h
		storage:set_string("center", minetest.write_json(map_maker.config.center))
	end
	...
	if fields.place_barrier then
		...
		ctf_map.emerge_with_callbacks(player_name, pos1, pos2, function()
			...
			minetest.after(0.1, function()
				ctf_map.place_middle_barrier(map_maker.config.center, map_maker.config.barrier_r,
						map_maker.config.center.h, (map_maker.config.barrier_rot == 0) and "x" or "z")
				show_progress_formspec(player_name, "Placing outer barriers, this may take a while...")
				minetest.after(0.1, function()
					ctf_map.place_outer_barrier(map_maker.config.center, map_maker.config.barrier_r, map_maker.config.center.h)
					show_gui(player_name)
				end)
			end)
		end, emerge_progress)
	...
	if fields.export then
		...
		local path = minetest.get_worldpath() .. "/schems/" .. map_maker.config.mapname .. "/"
		minetest.mkdir(path)
		...
		meta:set("name", map_maker.config.maptitle)
		meta:set("author", map_maker.config.mapauthor)
		if map_maker.config.mapinitial ~= "" then
			meta:set("initial_stuff", map_maker.config.mapinitial)
		end
		meta:set("rotation", map_maker.config.barrier_rot)
		meta:set("r", map_maker.config.center.r)
		meta:set("h", map_maker.config.center.h)

		for _, flags in pairs(map_maker.config.flags) do
			local pos = vector.subtract(flags, map_maker.config.center)
			if map_maker.config.barrier_rot == 0 then
		...
		minetest.after(0.1, function()
			...
			if minetest.create_schematic(worldedit.pos1[player_name],
					...
					filepath) then
				minetest.chat_send_all("Exported " .. map_maker.config.mapname .. " to " .. path)
